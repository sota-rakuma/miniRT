...
{
	const t_vec3d	o_to_point = vec3d_sub(shape->point, o);
	const double	a = vec3d_dot(shape->normal, o_to_screen);
	const double	b = vec3d_dot(shape->normal, o_to_point);
...
t_abc	equation_cylinder(t_vec3d o_to_screen, t_vec3d o, t_shape *shape)
{
	...
	return ((t_abc){
		...
		2.0 * (
			vec3d_dot(d_cross_v, o_cross_v) - vec3d_dot(d_cross_v, c_cross_v)),
		vec3d_dot(o_cross_v, o_cross_v) - 2.0
		* vec3d_dot(o_cross_v, c_cross_v)
		+ vec3d_dot(c_cross_v, c_cross_v)
		- shape->radius * shape->radius
	});
}

double	get_t_with_cylinder(
	t_vec3d o_to_screen, t_vec3d o, t_shape *shape, t_abc abc)
{
	...
	t1 = (-abc.b - sqrt(discriminant)) / (2 * abc.a);
	c_to_point = vec3d_sub(
			vec3d_add(o, vec3d_mult(o_to_screen, t1)), shape->center);
